...
PHISHING_HASHES_FILE = "processed_phishing_hashes.pkl"
...
def load_processed_hashes():
    """Charge les hashes déjà traités depuis un fichier"""
    if os.path.exists(PHISHING_HASHES_FILE):
        try:
            with open(PHISHING_HASHES_FILE, 'rb') as f:
                return pickle.load(f)
        except Exception as e:
            logger.error(f"Erreur lors du chargement des hashes: {str(e)}")
            return set()
    return set()

def save_processed_hashes(hashes):
    """Sauvegarde les hashes traités dans un fichier"""
    try:
        with open(PHISHING_HASHES_FILE, 'wb') as f:
            pickle.dump(hashes, f)
    except Exception as e:
        logger.error(f"Erreur lors de la sauvegarde des hashes: {str(e)}")

def process_phishing_alerts(processed_hashes):
    ...
    for alert in alerts:
        ...
        if alert_hash in processed_hashes:
            logger.debug(f"Alerte déjà traitée (hash: {alert_hash})")
            continue
        ...
        try:
            ...
            alerts_processed += 1
            logger.info(f"Email phishing envoyé pour alerte (hash: {alert_hash})")
        except Exception as e:
            logger.error(f"Erreur lors de l'envoi d'email phishing: {str(e)}")
    
    # Fusionner les anciens et nouveaux hashes
    all_hashes = processed_hashes.union(new_hashes)
    if new_hashes:
        save_processed_hashes(all_hashes)
    
    logger.info(f"Alertes de phishing traitées: {alerts_processed}")
    return all_hashes
...
def main():
    ...
    logger.info(f"Démarrage avec {len(processed_hashes)} hashes déjà traités")
    ...
    processed_hashes = process_phishing_alerts(processed_hashes)

    duration = (datetime.now() - start_time).total_seconds()
    logger.info(f"Traitement terminé en {duration:.2f} secondes")
    logger.info(f"Nombre total d'alertes créées : {alert_count}")
    logger.info(f"Total hashes phishing connus: {len(processed_hashes)}")
